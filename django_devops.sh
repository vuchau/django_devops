...
django_source() {
	...
	if [ "_$DJANGO_METHOD" = "_script" ]; then
		DJANGO_SOURCE="https://raw.githubusercontent.com/thuongdinh/django_devops/v0.1.4/django_devops.sh"
	elif [ "_$DJANGO_METHOD" = "_script-ddevops-exec" ]; then
		DJANGO_SOURCE="https://raw.githubusercontent.com/thuongdinh/django_devops/v0.1.4/django_devops-exec"
...
install_from_git() {
	...
	cd "$DJANGO_DEVOPS_DIR" && git checkout --quiet v0.1.4 && git branch --quiet -D master >/dev/null 2>&1
